...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', default='model', help='Model name to identify different logs/checkpoints')
    ...
    parser.add_argument('--multi_gpu', default=False, type=bool, help="Use multiple GPUs for multi-model training, one GPU per model")
    ...
    parser.add_argument('--patience', type=int, default=2, help="Early stopping: stop after N epochs without improvement")
    ...
    parser.add_argument('--eval_memsize', type=int, default=5, help="Approximate amount of avalable memory on GPU, used for optimal evaluation batch size calculation ")
    parser.add_argument('--gpu', default=0, type=int, help='GPU instance to use')
    parser.add_argument('--gpu_allow_growth', default=False, type=bool, help='Allow to gradually increase GPU memory usage instead of grabbing all available memory at start')
    parser.add_argument('--save_best_model', default=False, type=bool, help='Save best model during training. Requires do_eval=True')
    parser.add_argument('--forward_split', default=True, type=bool, help='Use walk-forward split for model evaluation. Requires do_eval=True')
    parser.add_argument('--side_split', default=False, type=bool, help='Use side split for model evaluation. Requires do_eval=True')
    parser.add_argument('--do_eval', default=True, type=bool, help='Periodically evaluate model quality during training')
    parser.add_argument('--write_summaries', default=True, type=bool, help='Write Tensorflow summaries')
    parser.add_argument('--verbose', default=True, type=bool, help='Print additional information during graph construction')
    parser.add_argument('--asgd_decay', type=float,  help="EMA decay for averaged SGD. Not use ASGD if not set")
    parser.add_argument('--tqdm', default=True, type=bool, help="Use tqdm for status display during training")
